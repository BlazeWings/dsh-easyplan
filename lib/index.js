 ...
 * 本文件做三件事：
 *
 *  0. 【dsh-easyplan 新增】HTML 展示页发出之前，注入 `lib/picker.browser.js`：
 *     用户悬停即见元素名、点击即表态，回执 postMessage 回卡片——
 *     第 2 阶段「改初稿」因此不需要页面写任何代码。
 *
 *  1. 注册工具 `easyplan_show`：agent 调它，就等于"把工作区里这一页 HTML 展示给用户"。
 *     这是本插件存在的理由——一个快捷入口，而不是又一个文件预览器。
 *
 *  2. 注册镜像读路由 `/api/easyplan/raw/<sessionId>/<工作区相对路径>`：
 *     它按工作区目录结构镜像分发文件，于是展示页里的相对引用天然可用
 *     （`<img src="stills/a.png">`、同目录的 `.css` / `.js`、`shot-f3.html`），
 *     写页面的 agent 不需要知道任何 URL 前缀。同一个地址也是
 *     "在浏览器打开原文件" 的目标。
 *
 * 另外提供 `POST /api/easyplan/feedback`，把用户在页面上的表态落到
 * `<工作区>/.dsh/easyplan/inbox.jsonl`（只追加），让反馈能快速回到 agent 手里。
 *
...
export const name = 'easyplan'

/** 需要的宿主服务：`tools` 注册工具，`webServer` 注册镜像路由。 */
export const inject = ['tools', 'webServer']

/** 本插件独占的路由前缀。 */
const ROUTE_PREFIX = '/api/easyplan'
...
/** 反馈落盘目录（工作区相对路径）。 */
const FEEDBACK_DIR = '.dsh/easyplan'
 ...
 * 插件包里要落到工作区的资产。
 *
 * 每一组：源目录（相对本模块，随包走）→ 工作区相对目录 → 文件清单。
 * 落地是 **create-only**，不覆盖用户改过的文件。
 */
const ASSET_SETS = [
  {
    source: '../styles/',
    target: '.dsh/easyplan/presets',
    files: ['index.json', 'soft.css', 'swiss.css', 'brutal.css', 'blueprint.css'],
  },
  {
    source: '../templates/',
    target: '.dsh/easyplan/templates',
    files: ['README.md', 'core.js', 'review.html', 'pick.html', 'report.html'],
  },
]
...
// ─────────────────────── 点选注入（dsh-easyplan 新增） ───────────────────────

/**
 * 注入标记：已在页面里出现过就不再注入（防止重复）。
 * 这个字符串同时出现在注入的 <script> 标签上。
 */
const PICKER_MARKER = 'data-easyplan-picker'

/** 点选脚本源码缓存：从包内读一次，之后每次注入复用。 */
let pickerSource = null

/**
 * 读出点选脚本（lib/picker.browser.js）的源码。
 * 读不到时返回 null——注入失败不该拖垮展示本身。
 * @returns 脚本源码，或 null。
 */
async function loadPickerSource() {
  if (pickerSource !== null) return pickerSource
  try {
    pickerSource = await readFile(new URL('./picker.browser.js', import.meta.url), 'utf8')
  } catch {
    return null
  }
  return pickerSource
...
/**
 * 把点选脚本注入 HTML 正文：优先插在 `</body>` 之前，没有 </body> 就追加在末尾。
 *
 * 只做注入，不做编号——编号由脚本在浏览器里对活 DOM 完成（宿主没有 DOM，
 * 用正则解析 HTML 脆弱且不可靠；OpenDesign 的 host 端标注在它的架构里必要，
 * 在这里不必要）。
 *
 * @param html - 页面正文。
 * @param source - picker.browser.js 的源码。
 * @returns 注入后的正文；已注入过则原样返回。
 */
function injectPicker(html, source) {
  if (html.includes(PICKER_MARKER)) return html
  const snippet = `\n<script ${PICKER_MARKER}>\n${source}\n</script>\n`
  const match = /<\/body\s*>/i.exec(html)
  if (match) return html.slice(0, match.index) + snippet + html.slice(match.index)
  return html + snippet
}
...
/**
 * 会话 → 工作区根。**权威来源是 `easyplan_show` 工具自己**：它拿到的是
...
async function workspaceRootFor(ctx, sessionId) {
  ...
  if (remembered !== undefined) return { root: remembered, source: 'easyplan_show' }
...
/**
 * 把插件包里的资产落到工作区，让页面能用相对路径引用它们：
 * 预设样式（`<link href="presets/soft.css">`）与模板（`templates/core.js`）。
 *
 * **create-only**：已存在的文件一律不碰——你在工作区里改过的预设/模板是你的，不该被覆盖。
 * 之所以由宿主做而不是让模型 `write`：前者零 token，后者每套要抄几千字。
 *
 * @param workspaceRoot - 会话工作区根。
 * @returns 本次新落地的 `目标目录/文件名`（已存在的不计）。
 */
async function seedAssets(workspaceRoot) {
  const created = []
  for (const set of ASSET_SETS) {
    const target = join(workspaceRoot, set.target)
    ...
    for (const file of set.files) {
      ...
      const content = await readFile(new URL(file, new URL(set.source, import.meta.url)), 'utf8')
      await writeFile(destination, content, 'utf8')
      created.push(`${set.target}/${file}`)
    }
  }
  return created
}
...
function makeRouteHandler(ctx) {
  return async function handle(request, response) {
    ...
    if (!tail.startsWith('/raw/')) {
      sendJson(response, 404, { ok: false, error: `unknown easyplan route: ${tail}` })
      return
    }
    ...
    // HTML 展示页：注入点选脚本后再发（dsh-easyplan 的核心改动）。
    // 超过单页上限的文件不注入，退回原始流式分发。
    if (contentType.startsWith('text/html') && method === 'GET' && info.size <= MAX_PAGE_BYTES) {
      const source = await loadPickerSource()
      if (source !== null && !source.includes('</' + 'script')) {
        try {
          const html = await readFile(located.path, 'utf8')
          const transformed = injectPicker(html, source)
...
/**
 * `POST /api/easyplan/feedback`：把用户在一个展示页上的表态追加进
 * `<工作区>/.dsh/easyplan/inbox.jsonl`，agent 读这个文件就能拿到反馈。
 * @param ctx - 宿主插件上下文。
 * @param request - 入站请求。
 * @param response - 响应。
 */
...
function receiptHint(html) {
  ...
  // dsh-easyplan 起，宿主给每个展示页自动注入点选脚本：任何结构元素都能被
  // 悬停点名（自动编号 path-x-y-z）、点击表态、回执 postMessage 给卡片——
  // 「能回话」不再依赖页面自己实现。这里只提醒一件脚本替代不了的事：
  // 如果页面摆的是几个**离散候选**（第 1 阶段：挑方案/挑素材），
  // 给每个候选一个**有意义的名字**，回执才读得懂（`hero-v3 同意` 比
  // `path-1-2 同意` 好使得多）。
  if (!hasIds) {
    return [
      'ⓘ Host note: the click-to-comment picker was injected into this page automatically,',
      'so the user CAN already point at any structural element (auto-numbered path-x-y-z).',
      'But this page has no authored ids (`data-id="…"` or similar) on its items.',
      'If the page presents discrete choices (variants, candidates, a plan to approve),',
      'give each one a meaningful, visible id — receipts like `hero-v3 OK` are far more',
      'useful to you than `path-1-2 OK`. Then call easyplan_show again.',
      'If the page genuinely has nothing to decide, say so in your reply instead — then it is fine.',
    ].join(' ')
  }

  return [
    'ⓘ Host note: the click-to-comment picker was injected automatically; element-level feedback',
    'needs no page code anymore. A postMessage hook-up of your own is only still needed when the page',
    'has custom verdict controls (the templates do this). Nothing to fix if that is intentional.',
  ].join(' ')
}
 ...
 *
 * 注意 `presets/*.css` 不会被误报：宿主在这次调用里刚把它们落地过（见 seedAssets）。
 *
 * @param html - 页面正文。
...
async function assetHint(html, pagePath) {
  ...
  return [
    ...
    'Write those files (relative to the page), fix the paths, or drop the references, then call easyplan_show again.',
  ].join(' ')
}

/**
 * 找出页面用 `<script src="…">` 引的**本地**脚本，返回它们的绝对路径（存在才收）。
 *
 * 回执体检必须把外链脚本一起算进来：模板外壳只有几百字节，
 * 交互逻辑全在 `templates/core.js` 里 —— 只看页面本体会把模板页**全部误报**
 * （实测：模板示例页就是这么被自己的检查咬到的）。
 *
 * @param html - 页面正文。
 * @param pagePath - 页面绝对路径（相对引用以它的目录为基准）。
 * @returns 存在的本地脚本绝对路径。
 */
async function localScriptPaths(html, pagePath) {
  const paths = []
  for (const match of html.matchAll(/<script[^>]*\bsrc\s*=\s*"([^"]+)"/gi)) {
    const value = match[1].trim()
    if (value === '') continue
    if (/^(?:[a-z][a-z0-9+.-]*:|\/\/|\/|#)/i.test(value)) continue
    let decoded
    try {
      ...
      decoded = value.split(/[?#]/)[0]
    }
    const full = resolve(dirname(pagePath), decoded)
    try {
      await access(full)
      paths.push(full)
    } catch {
...
/**
 * 手写的 `easyplan_show` 工具定义（见文件头说明为什么不用 `defineTool`）。
 *
...
function buildTool(ctx) {
  return {
    ...
    description: [
      ...
      'Load the `easyplan-report` skill BEFORE writing the page — it carries the reply contract, the four preset styles, and the sandbox traps. Do not write the page from memory.',
      'The host injects a click-to-comment picker into every shown page: the user can hover any element to see its name and click it to leave a verdict — no page code needed for that.',
      'What the page SHOULD still do: if it presents discrete choices (variants to pick, items to approve), give each a stable, visible, meaningful id (data-id="hero-v3"), so the user’s receipt names them readably. The skill says exactly how. If the page genuinely has nothing to decide, say so in your reply instead.',
    ...
    parameters: {
      ...
      properties: {
        path: {
          type: 'string',
          description: 'Workspace-relative path of an existing .html/.htm file, for example `.dsh/easyplan/review.html`.',
        },
        title: {
          type: 'string',
    ...
    async execute(args, exec) {
      ...
      if (relativePath === '') throw new Error('easyplan_show requires a non-empty `path`')

      const workspaceRoot = exec?.agent?.session?.header?.cwd
      if (typeof workspaceRoot !== 'string' || workspaceRoot === '') {
        throw new Error('easyplan_show requires an open workspace (the session has no cwd)')
      }
      ...
      if (!PAGE_EXTENSIONS.has(extension)) {
        throw new Error(`easyplan_show only shows .html/.htm pages; got "${extension || '(no extension)'}"`)
      }

      const located = await locateInWorkspace(workspaceRoot, relativePath)
      if (located.kind === 'escape') {
        throw new Error(`easyplan_show cannot read ${relativePath}: the path escapes the session workspace`)
      }
      if (located.kind === 'missing') {
        throw new Error(`easyplan_show cannot find ${relativePath} in the workspace. Write the page first, then call this again.`)
      }

      const info = await stat(located.path)
      if (!info.isFile()) throw new Error(`easyplan_show cannot show ${relativePath}: not a regular file`)
      if (info.size > MAX_PAGE_BYTES) {
        throw new Error(
          `easyplan_show cannot show ${relativePath}: ${info.size} bytes exceeds the ${MAX_PAGE_BYTES}-byte page limit`,
        )
      }
      ...
      // 只是页面不能外链预设。
      try {
        await seedAssets(workspaceRoot)
      } catch {
        /* 忽略 */
      }
      ...
      // 回执体检要把**外链脚本**一起扫——模板页的交互全在 templates/core.js 里，
      // 只看页面本体会把所有模板页误报（实测踩到过）。
      let hint = ''
      try {
        const html = await readFile(located.path, 'utf8')
        let scan = html
        for (const scriptPath of await localScriptPaths(html, located.path)) {
          try {
        ...
        const problems = [receiptHint(scan), await assetHint(html, located.path)].filter((one) => one !== '')
        hint = problems.join('\n\n')
      } catch {
        /* 读不到就只是不提醒，展示照常 */
      }
...
/**
 * 挂载镜像读路由与 `easyplan_show` 工具；两者都由 effect 持有，停用即净。
 * @param ctx - 宿主插件上下文。
 */
export function apply(ctx) {
  ctx.effect(
    ...
    'dsh-easyplan: workspace mirror route',
  )

  ctx.effect(() => ctx.tools.register(buildTool(ctx)), 'dsh-easyplan: easyplan_show tool')
}

/** 仅供离线测试触达的内部函数（Cordis 只认 name/inject/apply，多余导出无害）。 */
export const internals = { injectPicker, loadPickerSource }
