/**
 * easyplan-report · 模板内核（三个形状共用这一份）
 * ============================================================================
 *
...
(function () {
  ...
  /** 当前形状，由外壳用 `window.EASYPLAN_SHAPE` 声明。 */
  var SHAPE = window.EASYPLAN_SHAPE || 'review'
  ...
  function boot() {
    ...
    script.onload = function () {
      if (!window.EASYPLAN || typeof window.EASYPLAN !== 'object') {
        fail('数据文件里没有 `window.EASYPLAN`：' + name, [
          '数据文件必须写成 <code>window.EASYPLAN = { … }</code>；',
          '文件名必须和页面同名：<code>foo.html</code> ↔ <code>foo.data.js</code>。',
        ])
        return
      }
      try {
        render(window.EASYPLAN)
      } catch (error) {
    ...
    script.onerror = function () {
      fail('没找到数据文件：' + name, [
        ...
        '用 <code>write</code> 把数据写到 <code>.dsh/easyplan/' + esc(name) + '</code>。',
        '数据文件里第一行写 <code>window.EASYPLAN = { … }</code>。',
      ])
    }
