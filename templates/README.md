...
```bash
cp .dsh/easyplan/templates/review.html .dsh/easyplan/<名字>.html
# 然后 write .dsh/easyplan/<名字>.data.js
# 最后 easyplan_show({ path: '.dsh/easyplan/<名字>.html' })
```
...
| `shape` | string | 一般不用写——外壳已经声明了（`window.EASYPLAN_SHAPE`） |
...
| `kv` | `pairs`（`[键, 值]` 数组） | `{ type: 'kv', pairs: [['预设目录','.dsh/easyplan/presets/']] }` |
...
| 页面空白、没有内容 | 数据里没写 `window.EASYPLAN`，或 JSON 语法错了 |
