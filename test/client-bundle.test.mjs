...
check('加载器收到正确的包 id', () => assert.equal(loaded.id, 'dsh-easyplan'))
...
check('工具卡片绑在 easyplan_show 这个 key 上', () => {
  ...
  assert.equal(card.registration.key, 'easyplan_show')
...
check('全屏浮层注册在 shell.overlay 且带 id', () => {
  ...
  assert.equal(overlay.registration.id, 'dsh-easyplan-fullscreen')
})
check('输入镜像座位注册在 conversation.input.left', () => {
  ...
  assert.equal(input.registration.id, 'dsh-easyplan-input')
})
...
check('常规路径逐段编码、保留目录结构', () =>
  assert.equal(
    internals.rawUrl('sess-1', '.dsh/easyplan/review.html'),
    '/api/easyplan/raw/sess-1/.dsh/easyplan/review.html',
  ),
)
check('空格与中文被编码', () =>
  assert.equal(
    internals.rawUrl('s', '.dsh/easyplan/a b/图 1.png'),
    '/api/easyplan/raw/s/.dsh/easyplan/a%20b/%E5%9B%BE%201.png',
  ),
)
check('反斜杠被归一化（Windows 路径）', () =>
  assert.equal(internals.rawUrl('s', '.dsh\\easyplan\\a.html'), '/api/easyplan/raw/s/.dsh/easyplan/a.html'),
)
check('前导斜杠被吃掉，不产生空段', () =>
  assert.equal(internals.rawUrl('s', '/a/b.html'), '/api/easyplan/raw/s/a/b.html'),
)
check('sessionId 也被编码', () =>
  assert.equal(internals.rawUrl('a/b', 'x.html'), '/api/easyplan/raw/a%2Fb/x.html'),
)

console.log('\n[纯逻辑] 工具调用块解析')

check('running 形态：直接读 argsRaw', () => {
  const args = internals.readArgs({ callId: 'c1', name: 'easyplan_show', argsRaw: '{"path":"a.html"}' })
  assert.deepEqual(plain(args), { path: 'a.html' })
})
check('settled 形态：从 block.call 读', () => {
  const args = internals.readArgs({ kind: 'tool-result', call: { name: 'easyplan_show', argsRaw: '{"path":"b.html"}' } })
...
check('basename 兼容两种分隔符', () => {
  assert.equal(internals.basename('.dsh/easyplan/a.html'), 'a.html')
  assert.equal(internals.basename('.dsh\\easyplan\\b.html'), 'b.html')
})
...
const failedTree = card({
  block: {
    kind: 'tool-result',
    call: { name: 'easyplan_show', argsRaw: '{"path":".dsh/easyplan/x.html","title":"评审"}' },
    isError: true,
    content: [{ type: 'text', text: 'easyplan_show cannot find .dsh/easyplan/x.html in the workspace.' }],
  },
  sessionId: 's',
  openFile: () => {},
})
check('展示失败：把宿主的错误原文显示出来', () => {
  ...
  assert.match(text, /\.dsh\/easyplan\/x\.html/)
})

const okTree = card({
  block: {
    kind: 'tool-result',
    call: { name: 'easyplan_show', argsRaw: '{"path":".dsh/easyplan/x.html","title":"评审","note":"挑一个"}' },
    isError: false,
    meta: { bytes: 2048 },
...
check('正常态：没有 meta.url 时回退到用 sessionId 自己拼', () => {
  const anchor = findElement(okTree, (element) => element.type === 'a' && element.props.className === 'dsh-easyplan-btn')
  assert.ok(anchor !== null, '没有找到"浏览器打开"链接')
  assert.equal(anchor.props.href, '/api/easyplan/raw/sess-7/.dsh/easyplan/x.html')
...
const hostUrlTree = card({
  block: {
    kind: 'tool-result',
    call: { name: 'easyplan_show', argsRaw: '{"path":".dsh/easyplan/x.html"}' },
    isError: false,
    meta: { bytes: 10, url: '/api/easyplan/raw/host-side-id/.dsh/easyplan/x.html' },
    content: [],
  },
...
check('有 meta.url 时优先用宿主算的地址', () => {
  const anchor = findElement(hostUrlTree, (element) => element.type === 'a' && element.props.className === 'dsh-easyplan-btn')
  assert.equal(anchor.props.href, '/api/easyplan/raw/host-side-id/.dsh/easyplan/x.html')
})
check('反馈类型常量与文档一致', () => assert.equal(internals.FEEDBACK_TYPE, 'easyplan-feedback'))
