 ...
 * 回环围栏、反馈落盘，以及 `easyplan_show` 工具的定义合法性与参数校验。
 *
...
const workspace = await mkdtemp(join(tmpdir(), 'easyplan-ws-'))
const outside = await mkdtemp(join(tmpdir(), 'easyplan-out-'))

/** 工作区内容：一个展示页 + 同目录资源 + 一张图（相对引用要能解析）。 */
await mkdir(join(workspace, '.dsh', 'easyplan'), { recursive: true })
await mkdir(join(workspace, 'stills'), { recursive: true })
const pageBody = '<h1>hi</h1>'
await writeFile(join(workspace, '.dsh', 'easyplan', 'review.html'), pageBody, 'utf8')
 ...
 */
const escapeTargetName = `easyplan-escape-target-${Date.now()}.html`
...
assert.ok(captured.tool !== null, 'easyplan_show 工具没有注册')
...
const base = '/api/easyplan'
const raw = (relative) => `${base}/raw/live-session/${relative}`

console.log('\n[路由] 正常分发')

const page = await rawRequest(raw('.dsh/easyplan/review.html'))
...
check('正文包含原内容', () => assert.ok(page.body.includes(pageBody)))
check('展示页被注入点选脚本', () => assert.ok(page.body.includes('data-easyplan-picker')))
check('点选脚本在 </body> 之前或文末', () => assert.ok(page.body.indexOf('data-easyplan-picker') > pageBody.length - 1))

const head = await rawRequest(raw('.dsh/easyplan/review.html'), { method: 'HEAD' })
...
// 在 Linux/macOS 上反斜杠不是分隔符，此时 realpath 找不到文件，正确结果是 404。
const encodedBackslash = await rawRequest(raw(`%2e%2e%5c${escapeTargetName}`))
const expectedBackslash = process.platform === 'win32' ? 403 : 404
check(`编码反斜杠穿越（%5c）→ ${expectedBackslash}（按平台）`, () =>
  assert.equal(encodedBackslash.status, expectedBackslash),
)
...
const missing = await rawRequest(raw('.dsh/easyplan/nope.html'))
...
const metaShape = await rawRequest(`${base}/raw/meta-session/.dsh/easyplan/review.html`)
...
const badMethod = await rawRequest(raw('.dsh/easyplan/review.html'), { method: 'DELETE' })
...
const sandboxedFrame = await rawRequest(raw('.dsh/easyplan/review.html'), {
...
const sandboxedNavigation = await rawRequest(raw('.dsh/easyplan/review.html'), {
...
const opaqueOrigin = await rawRequest(raw('.dsh/easyplan/review.html'), {
...
const crossSiteFetch = await rawRequest(raw('.dsh/easyplan/review.html'), {
...
const crossSiteDocument = await rawRequest(raw('.dsh/easyplan/review.html'), {
...
const badHost = await rawRequest(raw('.dsh/easyplan/review.html'), { headers: { host: '10.0.0.5:3080' } })
...
const inbox = await readFile(join(workspace, '.dsh', 'easyplan', 'inbox.jsonl'), 'utf8')
...
console.log('\n[工具] easyplan_show 定义与执行')

const tool = captured.tool
check('工具名是 easyplan_show', () => assert.equal(tool.name, 'easyplan_show'))
...
const ok = await tool.execute({ path: '.dsh/easyplan/review.html' }, exec)
check('execute 返回 path 与 bytes', () =>
  assert.equal(ok.path, '.dsh/easyplan/review.html') && assert.equal(ok.bytes, Buffer.byteLength(pageBody)),
)
...
check('裸页面（没有命名 id）会收到"给候选起名"的提醒', () => {
  assert.match(ok.hint, /Host note/)
  assert.match(ok.hint, /authored ids/)
  assert.match(ok.hint, /discrete choices/)
})
...
check('render 把提醒带给模型；没有提醒时不夹带', () => {
  ...
  assert.ok(!/Host note/.test(clean[0].text), '干净页面不该被夹带提醒')
})

const POST = '<script>parent.postMessage({type:"easyplan-feedback",text:""},"*")</script>'

await writeFile(
  join(workspace, '.dsh', 'easyplan', 'with-receipt.html'),
  `<div class="card" data-id="pain-a">x</div><textarea readonly></textarea>${POST}`,
  'utf8',
)
const withReceipt = await tool.execute({ path: '.dsh/easyplan/with-receipt.html' }, exec)
check('id + postMessage 都有的页面不提醒', () => assert.equal(withReceipt.hint, ''))

// dsh-easyplan 起，postMessage 通道由注入的点选脚本兜底；有命名 id 但页面
// 自己没写 postMessage 时，只给一条"这是有意为之就忽略"的轻提示。
await writeFile(
  join(workspace, '.dsh', 'easyplan', 'copy-only.html'),
  '<div class="card" data-id="pain-a">x</div><textarea readonly></textarea>'
    + '<button>全选下面的文本</button>',
  'utf8',
)
const copyOnly = await tool.execute({ path: '.dsh/easyplan/copy-only.html' }, exec)
check('有 id 没自写 postMessage：只给注入已兜底的轻提示', () => {
  assert.match(copyOnly.hint, /picker was injected automatically/)
  assert.ok(!/authored ids/.test(copyOnly.hint), 'id 没缺，不该再提醒起名')
})

// 反向的一半：走了通道，但没有可点名的 id。
await writeFile(
  join(workspace, '.dsh', 'easyplan', 'no-ids.html'),
  `<textarea readonly></textarea>${POST}`,
  'utf8',
)
const noIds = await tool.execute({ path: '.dsh/easyplan/no-ids.html' }, exec)
check('有通道但候选没起名：提醒给候选一个有意义的 id', () => {
  assert.match(noIds.hint, /authored ids/)
  assert.match(noIds.hint, /hero-v3 OK/)
})
...
await writeFile(
  join(workspace, '.dsh', 'easyplan', 'kernel.js'),
  'var html = \'<div data-id="a">x</div>\'; '
    + 'parent.postMessage({ type: "easyplan-feedback", text: html }, "*");',
  'utf8',
)
await writeFile(
  join(workspace, '.dsh', 'easyplan', 'shell.html'),
  '<div class="page"><div id="sm-content"></div></div><script src="kernel.js"></script>',
  'utf8',
)
const shell = await tool.execute({ path: '.dsh/easyplan/shell.html' }, exec)
check('回执在外链脚本里时通过（模板页就是这种形状）', () => assert.equal(shell.hint, ''))

await writeFile(
  join(workspace, '.dsh', 'easyplan', 'kernel-empty.js'),
  'console.log("这里什么都没有")',
  'utf8',
)
await writeFile(
  join(workspace, '.dsh', 'easyplan', 'shell-empty.html'),
  '<div class="page"><div id="sm-content"></div></div><script src="kernel-empty.js"></script>',
  'utf8',
)
const shellEmpty = await tool.execute({ path: '.dsh/easyplan/shell-empty.html' }, exec)
check('外链脚本里也没有回执时，照样被点名', () => assert.match(shellEmpty.hint, /Host note/))
...
await mkdir(join(workspace, '.dsh', 'easyplan', 'stills'), { recursive: true })
await writeFile(join(workspace, '.dsh', 'easyplan', 'stills', 'missing.png'), 'x', 'utf8')
const fixed = await tool.execute({ path: '.dsh/easyplan/dangling.html' }, exec)
check('把文件补上之后就不再提醒', () => assert.equal(fixed.hint, ''))

await writeFile(
  join(workspace, '.dsh', 'easyplan', 'abs-refs.html'),
  `<div data-id="a">x</div><textarea readonly></textarea>${POST}`
    + '<a href="https://example.com/x.html">外链</a><a href="#top">锚点</a><img src="/api/x.png">',
  'utf8',
)
const absRefs = await tool.execute({ path: '.dsh/easyplan/abs-refs.html' }, exec)
check('绝对 / 协议 / 锚点引用不参与检查', () => assert.equal(absRefs.hint, ''))

// ── 预设样式落地（create-only） ──────────────────────────────────────────
console.log('\n[工具] 预设样式落地')

const presetDir = join(workspace, '.dsh', 'easyplan', 'presets')
const presetIndex = JSON.parse(await readFile(join(presetDir, 'index.json'), 'utf8'))
check('easyplan_show 顺带把预设清单落到工作区', () => assert.equal(typeof presetIndex.default, 'string'))
...
await tool.execute({ path: '.dsh/easyplan/review.html' }, exec)
...
const cases = [
  ...
  [{ path: '.dsh/easyplan/nope.html' }, exec, /cannot find/],
  [{ path: `../${escapeTargetName}` }, exec, /escapes the session workspace/],
  [{ path: 'decoy.html' }, exec, /not a regular file/],
  [{ path: '.dsh/easyplan/review.html' }, { agent: { session: { header: {} } } }, /requires an open workspace/],
]
...
const bareBase = `http://127.0.0.1:${bareServer.address().port}/api/easyplan`
/**
 * 对"服务全缺"那个服务器发一次请求。
 * @param pathname - /api/easyplan 之后的路径。
 */
...
const beforeRecord = await bareGet('/raw/fresh-session/.dsh/easyplan/review.html')
...
const recorded = await bare.tool.execute({ path: '.dsh/easyplan/review.html' }, bareExec)
check('工具执行时把 会话→工作区根 记了下来', () => assert.equal(recorded.bytes, Buffer.byteLength(pageBody)))
check('工具一并算好了给卡片用的地址', () =>
  assert.equal(recorded.url, '/api/easyplan/raw/fresh-session/.dsh/easyplan/review.html'),
)

const afterRecord = await bareGet('/raw/fresh-session/.dsh/easyplan/review.html')
check('记录之后路由就能服务了（即使服务全缺）', () => {
  assert.equal(afterRecord.status, 200)
  assert.ok(afterRecord.body.includes(pageBody)) && assert.ok(afterRecord.body.includes('data-easyplan-picker'))
})
...
const neverShown = await bareGet('/raw/never-shown/.dsh/easyplan/review.html')
