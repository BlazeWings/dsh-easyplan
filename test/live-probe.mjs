 ...
 *
 * 例：
 *   node test/live-probe.mjs eec4016e-fa4a-4345-a3b7-4c0fe7f825af .dsh/easyplan/review.html
 */
...
const prefix = `/api/easyplan/raw/${encodeURIComponent(sessionId)}/`

/** 默认探针：一个展示页、一个相对引用的资源、一次越界尝试。 */
const targets = process.argv.length > 3
  ? process.argv.slice(3).map((path) => ({ label: path, path, expect: 200 }))
  : [
      { label: '展示页', path: '.dsh/easyplan/review.html', expect: 200 },
      { label: '相对引用的 SVG', path: '.dsh/easyplan/assets/arch.svg', expect: 200 },
      { label: '越界（../ 到工作区外）', path: '%2e%2e%2fdsh-text-drop%2fpackage.json', expect: 403 },
      { label: '不存在的文件', path: '.dsh/easyplan/nope.html', expect: 404 },
